...
//        console.warn('Insufficient types')
//      }
    ...
    function typeCheckReturns(returnType, fnBody) {
      return traverse(fnBody, function (node, parent) {
        if (node.type !== 'ReturnStatement') {
          return node
        }

        return typeCheckReturn(returnType, node.argument)
      })
    }

    // TODO include actual tg
    var includeTG = [{
        ...
        return AssignmentExpression(
          ...
          FunctionExpression(
            ...
            typeCheckArgs(Store.types[name].params, fn.params)
              .concat(typeCheckReturns(Store.types[name].retval, fnBody))
          )
        )
      }
