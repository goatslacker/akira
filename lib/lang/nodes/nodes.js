...
Nodes.prototype = {
  ...
  addContracts: function (tree) {
    ...
    function typeCheckArgs(argTypes, args) {
      var typedArgs = traverse(argTypes, function (node) {
        if (node.type == 'Identifier' && Store.types.hasOwnProperty(node.name)) {
          return Store.types[node.name].retval
        } else {
          return node
        }
      })
      ...
      return [{
        ...
        expression: CallExpression(
          Identifier('_$tg'),
          [ArrayExpression(typedArgs), ArrayExpression(args)]
        )
      }]
    }
      ...
      if (hasTypeCheck(node)) {
        ...
        return AssignmentExpression(
          ...
          FunctionExpression(
            null,
            fn.params,
            typedArgs.concat(typeCheckReturn(Store.types[name].retval, fnBody))
          )
        )
      }
