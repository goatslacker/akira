...
Nodes.prototype = {
  ...
  addCore: function () {
    ...
    var result = Object.keys(core).map(function (key) {
      var exists = fs.existsSync('./lib/core/' + key + '.js')
      if (exists) {
        var data = String(fs.readFileSync('./lib/core/' + key + '.js'))
        return esprima.parse(data).body[0]
      }
      return core[key];
    });
    Store.core = {};
  ...
  compile: function (context, filename) {
    ...
    var nodes = fu.concatMap(function (node) {
