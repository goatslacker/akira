...
function wrap(exp) {
  if (Array.isArray(exp) || exp.type === 'ExpressionStatement') {
    return exp
  }
...
function makeExpression(expr, a, context) {
  ...
  return wrap(compiledExpr).concat(wrap(a))
}
...
function addNextToCall(call, boundName, next) {
  return call.type === 'CallExpression'
    ? CallExpression(call.callee, call.arguments.concat(
        FunctionExpression(null, boundName.elements, wrap(next))
      ))
    : call
}
...
Async.prototype.compile = function (context) {
  ...
  return createFunction(args, wrap(body), scope)
}
