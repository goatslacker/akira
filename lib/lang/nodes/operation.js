...
function math(op, ops, context, line) {
  var left = Base.compileValue(ops[0], context);
  var right = Base.compileValue(ops[1], context);
  ...
  switch (op) {
    ...
    case '*':
    case '/':
      Base.checkType(left, 'number', context, line);
      Base.checkType(right, 'number', context, line);
      break;
  }
