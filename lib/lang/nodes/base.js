...
Base.destructure = function (arg, elements, idx, fn, context) {
  var offset_identifier = {
    type: 'Identifier',
    name: '__offset' + idx
  };
  var tmpvar = '__ds' + idx;

  var destructured = elements
      ...
      if (node.__rest) {
        ...
        action = {
          ...
          property: {
            ...
            operator: '++',
            argument: offset_identifier,
            prefix: false
          }
        };
      }

      return context[node.name] = Base.assignment(context, node, action);
    });

  var offset = [];

  if (!context[offset_identifier.name] && destructured.length > 0) {
    offset = [Base.assignment(
      context,
      offset_identifier,
      { type: 'Literal', value: 0 }
    )];
    context[offset_identifier.name] = { type: 'Literal', value: 0 };
  }

  return offset.concat(destructured);
}
