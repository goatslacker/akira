...
Base.VariableDeclaration = js('VariableDeclaration');

Base.genUniqueIdentifier = function (init, incrementor) {
...
Base.obj2str = function obj2str(val) {
  ...
  if (object.type === 'ObjectExpression') {
    object.properties.forEach(function (property) {
      if (property.key.name === val) {
        Base.checkType(property.value, expected, context, line);
      }
    });
  }
}
...
Base.checkType = function checkType(item, expected, context, line) {
  function error(typesig) {
    var value = typesig ||
      (item.type === 'Literal' ? get_type(item.value) : item.type);
  ...
  switch (item.type) {
    ...
    case 'Literal':
      ...
      break;
    case 'CallExpression':
      if (item.callee.type === 'Identifier' && Base._types[item.callee.name]) {
        if (Base._types[item.callee.name].retval.name.toLowerCase() !==
            expected) {
          error(Base._types[item.callee.name].retval.name);
        }
      }
      break;
