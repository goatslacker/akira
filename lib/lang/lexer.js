...
!function() {
var find=function(f,ds){for(var i in ds){if(f(ds[i])===true){return i;}}};var last=function(xs){return xs[xs.length-1];};
var rx, identifiers, ignore, getType, extract, parseAlphanumeric, returnIdentifier, parseIdentifier, convertRegexp, parseRegularExpressions, parseArgs, special, parseStr, getToken, processChunk, incrLine, getNextIndex, returnTokens, filterWhitespace, rxindent, tokenize;
rx = {
...
getToken = function (type, chunk) {
  return function () {
    switch (false) {
    case !(type === 'IDENTIFIER' || type === 'KEY' || type === 'THIS' || type === 'OPERATION'):
...
tokenize = function (code) {
  var lines, tokens, indentstack, dedentLoop, dedent;
  lines = code.split('\n');
  tokens = [];
  indentstack = [];
  dedentLoop = function (f, i) {
    var dedentNum;
    return function () {
      if (indentstack.length && f()) {
        dedentNum = indentstack.pop();
        tokens.push([
          'DEDENT',
          dedentNum,
          ' ',
  ...
  dedent = function (f, i) {
    return indentstack.length && f() ? dedentLoop(f, i) : null;
  };
  lines.forEach(function (line, i) {
    var indentation, indent, lastIndent;
    indentation = rx.COMMENTS.test(line) ? null : rxindent.exec(line);
    indent = indentation ? indentation[0].length : null;
    (function () {
      if (indent) {
        return function () {
          if (indentstack.length) {
            lastIndent = last(indentstack);
            return function () {
              switch (false) {
              ...
              case !(indent < lastIndent):
                return dedent(function () {
                  return indentstack.length && indent < last(indentstack);
                }, i);
              default:
                return null;
              }
        ...
        return line !== '' && !rx.COMMENTS.test(line) ? dedent(function () {
          return indentstack.length;
        }, i) : null;
      }
    }.apply(this, arguments));
    tokens.push.apply(tokens, returnTokens(line, {
      index: 0,
      line: i
