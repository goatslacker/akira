...
(function () {
  var rx, identifiers, ignore, getType, extract, parseAlphanumeric, returnIdentifier, parseIdentifier, convertRegexp, parseRegularExpressions, parseArgs, getToken, processChunk, getLength, incrLine, getNextIndex, returnTokens, filterWhitespace, tokenize;
  ...
  getNextIndex = function (token) {
    return {
      index: getLength(token[0], token[2].toString().length),
      token: token
    };
  };
