...
var fs, path, util, vm, escodegen, me, lexer, Parser, parser, tokenize, parse, ast, wrap, compile, file, run, output, analyze, write, memory;
fs = require('fs');
path = require('path');
util = require('util');
vm = require('vm');
...
memory = function memory(item, action) {
    switch (false) {
    case !(action === 'compile'):
        return write(compile(file(item)), item);
    case !(action === 'code'):
