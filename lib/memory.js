...
$$MEMORY.exports = (function ($$export) {
    ...
    var fs, path, util, vm, escodegen, nodewatch, me, lexer, parser, tokenize, parse, ast, generate, wrap, compile, noWrap, walk, file, files, run, output, analyze, write, watch, displayHelp, startRepl, prompt, repl, memory;
    fs = require('fs');
    path = require('path');
    util = require('util');
    vm = require('vm');
    ...
    ast = function (parsed, name, context, evald) {
        var self, body, ast, xform, program;
        self = context || {};
        body = parsed.compile(self, name);
        ast = parsed.getUtils(self).concat(parsed.addVars(self, evald), body);
        xform = {
            type: 'CallExpression',
            callee: {
    ...
    wrap = function (code) {
        return [
            'var $$MEMORY = (typeof module !== "undefined") ? module : {};',
            '$$MEMORY.exports = '.concat(code)
        ];
    };
    compile = function (code, name, context, evald) {
        return wrap(noWrap(code, name, context, evald));
    };
    noWrap = function (code, name, context, evald) {
        return generate(ast(parse(tokenize(code)), name, context, evald));
    };
    ...
    startRepl = function () {
        ...
        context = {};
        repl(function (data) {
            print(eval(noWrap(data, 'repl', context, true)));
            return prompt();
        });
        return prompt();
    };
    ...
    return memory = function (action, item) {
        switch (false) {
        case !(action === 'compile'):
            return files(item, function (code, name) {
                return write(compile(code, item), name);
            });
        case !(action === 'code'):
            return output(compile(file(item), item));
        case !(action === 'test'):
            return run(compile(file('test/tests.mem'), 'test/tests.mem'));
        case !(action === 'run'):
            return run(compile(file(item), item));
        case !(action === 'tokens'):
            return analyze(tokenize(file(item)));
        case !(action === 'ast'):
            return analyze(ast(parse(tokenize(file(item))), item));
        case !(action === 'version'):
