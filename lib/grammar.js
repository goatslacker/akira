(function () {
var fs, jison, unwrap, bnf, tokens, operators, Parser, grammar, parser;
fs = require('fs');
jison = require('jison');
unwrap = /^function\s*\(\)\s*\{\s*return\s*([\s\S]*)\s*\}/;
var getMatch = function getMatch(action) {
    var x;
...
var addYY = function addYY(str) {
    return str.replace(/new /g, 'new yy.');
};
var o = function o(pattern, action) {
    var patternStringRegExp, patternString, newaction;
...
bnf = {
    ...
    PatternMatching: [
        o('Identifier ( Parameters ) = TERMINATOR Patterns', function () {
            return new Declaration($1, $3, new Pattern($3, $7));
        })
    ],
    Comma: [
        o(', OptTerminator')
