module.exports = (function ($$export) {
    var util, me, akira, run, lex, genAst, benchmark, writeFile, getFiles, startRepl, watch, wrap, output, analyse, MEM;
    var map;
    map = function (f, list) {
        return list.map(f);
    };
    ...
    var $$e243ba = function ($$export) {
            var vm, util, escodegen, getFiles, mark, lexer, parser, toAst, transform, lex, parse, genAst, transpile, benchmark, akira, run;
            vm = require('vm');
            util = require('util');
    ...
    return MEM = function (action, filename) {
        ...
        switch (false) {
        case !(action === 'bench'):
            return akira(analyse, filename, benchmark);
        case !(action === 'debug'):
            return akira(function () {
                return null;
            }, filename);
        case !(action === 'compile'):
            return akira(wrap, filename);
        case !(action === 'output'):
            return akira(output, filename);
        case !(action === 'test'):
            return akira(run, 'test/tests.akira');
        case !(action === 'run'):
            return akira(run, filename);
        case !(action === 'tokens'):
            return akira(analyse, filename, lex);
        case !(action === 'ast'):
            return akira(analyse, filename, genAst);
        case !(action === 'version'):
