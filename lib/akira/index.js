...
!function() {
var requireNodeAsync=function(m){return Promise.promisifyAll(require(m));};var Promise=require('bluebird');var show=function(i){return i.toString();};var flatten=function(vec,f){var result;if(f===void 0)f=Array.isArray;result=[];vec.forEach(function(x){return f(x)?Array.prototype.push.apply(result,flatten(x)):result.push(x);});return result;};function filter(f,a){if(arguments.length==1)return function(a){return filter(f,a);};var i=0,l=a.length,r=[];for(i;i<l;i+=1)f(a[i])===true&&r.push(a[i]);return r;}var print=function(args){args=Array.prototype.slice.call(arguments,0,arguments.length-0);return Object.prototype.toString.call(console.log)==='[object Function]'?console.log.apply(console,args):null;};function map(f,a){if(arguments.length==1)return function(a){return map(f,a);};var i=0,l=a.length,r=Array(l);for(i;i<l;i+=1)r[i]=f(a[i]);return r;}var head=function(_$fpds0){var x;x=_$fpds0[0];return x;};var tail=function(_$fpds0){var x,xs;x=_$fpds0[0];xs=_$fpds0.slice(1,_$fpds0.length-0);return xs;};
