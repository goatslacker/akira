...
Base.addScope = function (context, body) {
  var scope = {
    ...
    params = params.map(function (param) {
      ...
      if (Array.isArray(param)) {
        ...
        expressions.push({
          type: 'VariableDeclaration',
          kind: 'var',
          declarations: param.elements.map(function (val, index) {
            var action = '__list[' + index + ']';

            if (val.name === 'rest') {
              action = '__list.slice(' + index + ')';
            }
        ...
        return { type: 'Identifier', name: '__list' };
      }
