...
function identifier_check(item, self) {
  if (item.type !== 'Identifier') {
    throw new SyntaxError('Expected Identifier instead saw ' + item.type + ' at line ' + self.line);
  }
...
Assignment.immutable = function (symbol, value, context, self) {
  if (context.hasOwnProperty(symbol)) {
    throw new ReferenceError(symbol + ' is already defined at line ' + self.line);
  }
  ...
  switch (this.id.constructor.name) {
    ...
    case 'List':
      return destructor(context, value, this.id.list[0].args, function (arg, i) {
        var item = identifier_check(Base.compileValue(arg, context), self);
    ...
    case 'Tuple':
      return destructor(context, value, this.id.obj.args, function (arg) {
        var item = identifier_check(Base.compileValue(arg.id, context), self);

        return assign(context, item, {
          type: 'MemberExpression',
          computed: false,
          object: $$ref,
          property: identifier_check(Base.compileValue(arg.val, context), self)
        }, self);
      });
    default:
