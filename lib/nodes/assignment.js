...
function identifier_check(item) {
  if (item.type !== 'Identifier') {
    throw new SyntaxError('Expected Identifier instead saw ' + item.type);
  }
...
function assign(context, left, right) {
  Assignment.immutable(left.name, right, context);
...
function destructor(context, value, obj, fn) {
  ...
  return { type: 'BlockStatement', body: declarations.concat(body.concat(obj.map(fn))) };
}
...
Assignment.immutable = function (symbol, value, context) {
  if (context.hasOwnProperty(symbol)) {
    throw new ReferenceError(symbol + ' is already defined');
  }
  ...
  switch (this.id.constructor.name) {
    case 'Access':
      return assign(name, {
        ...
        operator: '||',
        left: Base.compileValue(this.id, context),
        right: value
      });
    case 'List':
      return destructor(context, value, this.id.list[0].args, function (arg, i) {
        var item = identifier_check(Base.compileValue(arg, context));
    ...
    case 'Tuple':
      return destructor(context, value, this.id.obj.args, function (arg) {
        var item = identifier_check(Base.compileValue(arg.id, context));

        return assign(context, item, {
          type: 'MemberExpression',
          computed: false,
          object: $$ref,
          property: identifier_check(Base.compileValue(arg.val, context))
        });
      });
    default:
      Assignment.immutable(name, value, context);
