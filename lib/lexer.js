...
var length = function (of) {
    return of.length;
};
var rx, identifiers, ignore, getType, extract, parseAlphanumeric, filterIdentifiersFn, returnIdentifier, parseIdentifier, parseRegularExpressions, getToken, processChunk, getLength, incrLine, returnTokens, filterWhitespace, tokenize;
rx = {
...
filterIdentifiersFn = function (iden) {
    return partial(filter, function (x) {
        return identifiers[x].indexOf(iden) >= 0;
    });
};
returnIdentifier = function (item, type) {
    switch (false) {
    case !(type === 'KEYWORDS'):
        return [
            item.toUpperCase(),
