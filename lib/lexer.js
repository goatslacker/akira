...
var getType = function getType(a) {
    var filterTokens = filterTokensFn(a);
...
var parseIdentifier = function parseIdentifier(t, chunk) {
    var item = extract(t, chunk);
    var filterIdentifiers = filterIdentifiersFn(item[1]);
    var token = pop(filterIdentifiers(Object.keys(identifiers)));
    return returnIdentifier(item[1], token || t);
};
var parseRegularExpressions = function parseRegularExpressions(t, chunk) {
    var item = extract(t, chunk);
    var regexp = new RegExp(item[2], item[3]);
    return [
        'REGEXP',
        regexp
    ];
...
var returnTokens = function returnTokens(code, i, line) {
    var tokens = [];
    var len = length(code);
    var chunk = code.substr(i, len);
    if (chunk) {
        var token = processChunk(chunk, line);
        tokens.push(token);
...
var tokenize = function tokenize(code) {
    ...
    var tokens = returnTokens(code.trim(), 0, 1);
